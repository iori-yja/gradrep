...
virtio は準仮想化デバイスのデファクトスタンダードとして広く用いられており、ライブラリ OS のライブラリでもサポートされる数少ないデバイスである。
...
\begin{tikzpicture}[scale=\tikzscale, ->, >=stealth', every node/.append style={draw}]
  ...
  \node (cap1) 
            [rectangle split, rectangle split parts=6, draw, right=2 of cfg]
...
ここで、\hyperlink{cap1}{最初の capability エレメント}は、 virtio の capability list の形をしている。
